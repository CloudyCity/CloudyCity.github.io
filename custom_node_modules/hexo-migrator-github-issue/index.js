...
function nextpage(cb) {
  var topPrefix = 'top_';
  var categoryPrefix = 'categories_';
      ...
      if (body && body.length) {
        for(var i in body) {
          ...
          for (var i in issue.labels) {
            var labelName = issue.labels[i].name;
            if (labelName.indexOf(categoryPrefix) != -1) { // 分类
              labelName = labelName.substr(categoryPrefix.length);
              categories.push(labelName);
            } else if (labelName.indexOf(topPrefix) != -1) { // 置顶
              labelName = labelName.substr(topPrefix.length);
              data.top = parseInt(labelName);
            } else {
              tags.push(labelName);
            }
          }
